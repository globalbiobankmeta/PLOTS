...
if(opt$maf != ""){
if( grepl(".gz$",opt$input) | grepl(".bgz$",opt$input) ) {
	gwas = fread(cmd=paste0("gunzip -c ", opt$input), select=c(opt$pvalue, opt$maf))
} else {
	gwas <- fread(opt$input, select=c(opt$pvalue, opt$maf))
}
}else{
if( grepl(".gz$",opt$input) | grepl(".bgz$",opt$input) ) {
        gwas = fread(cmd=paste0("gunzip -c ", opt$input), select=c(opt$pvalue, opt$af))
} else {
        gwas <- fread(opt$input, select=c(opt$pvalue, opt$af))
}
...
gwas$freqbin <- cut(gwas[[opt$maf]], freqbins,include.lowest=T)
  ...
  gwas$MAF = pmin(gwas[[opt$af]], 1-gwas[[opt$af]])
  gwas <- na.omit(data.frame(gwas[,c("MAF",ycol),with=F]))
minMAF <- min(gwas[["MAF"]])
...
gwas$freqbin <- cut(gwas[["MAF"]], freqbins,include.lowest=T)
  ...
  stop("No MAF or AF col is specified\n")
}
...
png(filename = paste0(opt$prefix,"_QQ.png"), width = opt$width, height = opt$height, pointsize = opt$pointsize)
	...
	if(maxY > opt$break.top){
		...
		plot(0,0,
			...
			main=paste("\nlambda ", quants, ": ", lambda, sep="" ),pch=19)
